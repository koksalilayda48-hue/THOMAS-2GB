...
# BOT & ENVIRONMENT
# ---------------------
TOKEN = os.environ.get("BOT_TOKEN")  # Render’da Environment Variable olarak ekle
...
def add_ref(ref_id, username=None):
    ...
    # Haftalık referans
    REFERANS_DATABASE["weekly"][uid] = REFERANS_DATABASE["weekly"].get(uid,0)+1
    # Username kaydı
    if username:
...
# ---------------------
# TOP WEEK KOMUTU (haftalık)
# ---------------------
@bot.message_handler(commands=["topweek"])
def top_week(message):
    weekly_refs = REFERANS_DATABASE.get("weekly",{})
    sorted_refs = sorted(weekly_refs.items(), key=lambda x: x[1], reverse=True)
    text = "📊 Bu haftanın en çok referans yapanlar:\n"
    for i, (uid,count) in enumerate(sorted_refs[:10],1):
        username = REFERANS_DATABASE["users"].get(uid,"Bilinmiyor")
        text += f"{i}. {username} ➜ {count} referans\n"
    bot.send_message(message.chat.id,text)
